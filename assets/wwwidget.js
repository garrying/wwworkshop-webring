(function () {
  const baseUrl = 'https://wwworkshop.org'
  const myElement = document.createElement('div')
  myElement.id = 'wwworkshop-widget'

  function getSyncScriptParams () {
    const scripts = document.getElementsByTagName('script')
    const scriptEles = Array.prototype.map.call(scripts, element => ({
      src: element.src,
      color: (element.dataset.color || '#000'),
      type: element.dataset.linkType
    }))
    const widgetEle = scriptEles.filter(script => script.src.includes('wwwidget.js'))[0]
